...
public class DonationRepository implements BasicRepository<Donation> {

    private final DatabaseManager databaseManager;

    public DonationRepository(DatabaseManager databaseManager) {
    ...
    public Optional<Donation> update(Donation data) throws SQLException {
        try (Connection connection = databaseManager.getConnection()) {
            ...
            if (data.charity() != null && data.charity().id() != null) stmt.setLong(1, data.charity().id()); else stmt.setNull(1, java.sql.Types.BIGINT);
            if (data.donor() != null && data.donor().id() != null) stmt.setLong(2, data.donor().id()); else stmt.setNull(2, java.sql.Types.BIGINT);
            if (data.amount() != null) stmt.setDouble(3, data.amount()); else stmt.setNull(3, java.sql.Types.DOUBLE);
